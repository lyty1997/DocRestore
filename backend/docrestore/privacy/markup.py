...
#: 须整体保护；再标签 / 图片 / 链接目标 / 数学 / URL）。各分支均不含捕获组，
#: 故 ``re.split`` 只产出唯一外层捕获组（奇数段=保护段）。
#:
#: 两处与隐私正确性强相关的顺序约束：
#: 1. 链接只保护 ``](目标)``、放出可见 label —— label 是正文，可能含人名/机构名，
#:    必须进入脱敏/NER；图片仍整段保护（alt 多为 OCR 文件名，放出反易误检为人名）。
#:    图片分支须列在链接分支前（图片以 ``!`` 起首，二者起始字符不同，顺序为冗余防御）。
#: 2. 独立公式 ``$$...$$`` 须列在行内 ``$...$`` 前，否则起始 ``$$`` 会被行内分支
#:    当作一对空 ``$$`` 先吃掉，导致公式正文落入自由段被替换/暴露给云端 NER。
_STRUCTURE_PATTERNS: tuple[str, ...] = (
    r"```.*?```",                 # 围栏代码块（DOTALL 跨行）
    r"`[^`]*`",                   # 行内代码
    r"<[^>]+>",                   # HTML 标签（含 <img src=...> / <td ...> / </td>）
    r"!\[[^\]]*\]\([^)]*\)",      # markdown 图片：alt+src 整段保护
    r"\]\([^)]*\)",               # markdown 链接：只保护 ](目标)，放出 label
    r"\$\$.*?\$\$",               # LaTeX 独立公式 $$...$$（须先于行内 $...$）
