...
- **并发与资源**：
  ...
  - 下游（CPU/网络）阶段默认串行执行（当前不接入 pipeline_semaphore）。
...
### 9.2 pipeline_semaphore 预留

当前 `Pipeline.process()` 不接入 `pipeline_semaphore`，下游阶段（去重/精修/输出）默认随任务串行执行。
后续如需限制“同时运行的 pipeline 数量”，应在 Scheduler 层引入/恢复统一的 semaphore 管控。
