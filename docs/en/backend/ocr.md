...
**GPU selection**: `OCRConfig.gpu_id` defaults to `None` (auto). `EngineManager.ensure()` calls `docrestore.ocr.gpu_detect.pick_best_gpu()` at the entry point to resolve it to a concrete physical index, then passes it to ppocr-server and DeepSeek worker via `CUDA_VISIBLE_DEVICES`. The recommendation prefers higher CUDA compute capability, then free VRAM, total VRAM, and physical index. The frontend fetches `GET /api/v1/gpus` (`{gpus, recommended}`) to render the selector; the "Auto (recommended)" option sends an empty string so the backend re-runs `pick_best_gpu` and remains authoritative.
