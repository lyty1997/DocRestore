...
**白平衡校正（必须，否则相机偏色被原样搬进 pptx）**：相机白平衡偏色（如偏蓝）会把**本该白的
背景拍成淡蓝**，直接采样会误把白底填成蓝色。`estimate_white_balance(arr)` **整页估一次**每通道
增益 `gain_c = 255 / 白点_c`（白点=每通道 95 百分位=背景白），把背景白映射回真白；增益限幅
`_WB_MAX_GAIN`、白点最亮通道 <`_WB_MIN_WHITE` 则 None（暗色主题不强制白化）。增益**只施于最终
输出色**（守卫仍用未校正色，阈值按真机标定不变）。校正后：偏蓝白底→近白（不填，§11.3）、真彩
banner→仍饱和（填）。
...
只设前景色时浅色字会「白底隐形」。规则：背景**视为白底**(`_is_effectively_white`：最暗通道≥200
**且**通道极差≤30，即浅且近中性)→不填充（slide 默认白底即可）；背景**深色 / 真彩色**→实心填充 +
无边框。暗色模式 / 彩色标题栏的浅字才可见，普通白底不画多余色块。**为何不用「各通道≥240」**：白平衡
校正后白底落在浅中性灰（如 `(210,224,229)`，<240 但浅且近中性），饱和度判别才能既吸收残留偏色、
又不误把真彩浅色（如浅蓝 `(200,220,255)`，极差 55）当白。渐变背景靠 §11.1「无主导桶→frac_bg 不足弃权」兜住。
