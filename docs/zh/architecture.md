...
- **PPT 还原模式（S0–S6 已落地）**：屏摄幻灯片 → 保序 `document.md`（文字 + 公式 LaTeX + 化学结构裁图），第三消费者分支 `_ppt_pipeline` 与文档/代码模式互斥三选一；设计见 [ppt-mode.md](ppt-mode.md)，子任务树 AGE-83
- **PDF 输入支持（Epic A 设计定稿，待实现）**：PDF 逐页渲染（pypdfium2）→ 复用现有 OCR→去重→精修链路，一个 PDF = 一篇文档；摄取入口渲染、单 PDF 落根命中快路、多 PDF 分子目录；设计见 [pdf-mode.md](pdf-mode.md)，子任务 `#75`(后端)/`#76`(前端)
