...
本方案基于 `/mnt/TrueNAS_Share/chromium/chromium_decode/code` 与
`/tmp/docrestore_b5950355` 的抽样排查，但设计目标不是 Chromium 专用，也不限定
C/C++。
...
- 参考源码只能是可选增强，不能把代码模式设计成 Chromium/C++ 专用恢复器。
...
目标：
- 提高多语言代码照片还原质量，不绑定 Chromium、VSCode 或 C/C++。
...
非目标：
- 不主动联网查找源码。
- 不在代码中写死任何 Chromium 路径、文件名、正文关键字或 C++ 专用规则作为通用逻辑。
