...
**GPU 选择**：`OCRConfig.gpu_id` 默认 `None`，表示"自动"。`EngineManager.ensure()` 入口会调 `docrestore.ocr.gpu_detect.pick_best_gpu()` 落地成具体物理索引，再通过 `CUDA_VISIBLE_DEVICES` 传给 ppocr-server 和 DeepSeek worker。自动推荐优先更高 CUDA compute capability，其次空闲显存、总显存和物理索引；前端任务表单调用 `GET /api/v1/gpus`（返回 `gpus + recommended`）渲染下拉；"自动（推荐）"项传空字符串，后端再次调 `pick_best_gpu` 保持权威。
